...
function [imageID] = JEC_no_class(FeaMat, Fea, test, FeaName, alpha, i, scope)
...
for j=1:size(Fea,2)
    ...
    if(strcmp(FeaName{j}, 'DenseHue') || strcmp(FeaName{j}, 'DenseSift'))
       dist = dist + GetL2Dist(FeaMat, tstMat(i,:), alpha(j)); 
    elseif(strcmp(FeaName{j}, 'Gist'))
        dist = dist + GetL2Dist(FeaMat, tstMat(i,:), alpha(j));
    else
        display('error distance metric!')
        pause;
    end
